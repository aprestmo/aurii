...
1. **SQL pushdown** for joins and COUNT — Phase 4 workstream D (**N4:** `COUNT(*)` landed; joins still in-memory)
2. **Reference indexes** and O(1) lookup by natural key during import — Phase 4 workstream D (**N4:** `findEntityByField` + `data.id` index)
3. **Dot-notation traversal** as planner rewrite to joins — candidate under D / later query work
4. **GROUP BY** and additional aggregates — Phase 4 workstream D (COUNT only; GROUP BY deferred)
